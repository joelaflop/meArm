...
ServoTimer2 sv0,sv1,sv2,sv3;   // nonclear arm
ServoTimer2 sv4,sv5,sv6,sv7;   //clear arm
double pr4,pr5,pr6,pr7;
double ps4,ps5,ps6,ps7;
double a0=PI/2,a1=PI/2,ra1=PI/2,a2=PI/2,fa2=PI/2;
...
void loop() 
{
  //uint8_t infolen = VW_MAX_MESSAGE_LEN;
...
void grab() //byte
{
  uint8_t infolen = VW_MAX_MESSAGE_LEN;
  uint8_t info[infolen];
  if(vw_get_message(info, &infolen))
  {
    Serial.print("   got:   ");
    x = info[0];
      Serial.print("    ");
...
void doallchuck()
{
  ...
  while(cbut==1)
  {
      ...
      if(abs(joyx-125)>25)
      {
        ...
        Serial.print("    CCX: ");Serial.print(CCX);Serial.print("    ");
      }
      if(abs(accx-125)>20)
      {
        //if(CCX<136)
      ...
      if(abs(joyy-125)>25)
      {
        if(CCY<145 && sqrt((CCZ*CCZ)+(CCY*CCY))<162)
          CCY += stepper*(joyy-125);
        Serial.print("    CCY: ");Serial.print(CCY);Serial.print("    ");
      }
      if(abs(accy-125)>20)
      {
        if(CCZ<140 && sqrt((CCZ*CCZ)+(CCY*CCY))<162)
...
void doallpot()
{
  ...
  //readandmappot();
  writepots();
  //printpots(); //this function is badddddddd
  //dothelcdpot();
  //plotpots();
}
...
void gotoangles(double A0,double A1,double A2, double A3)
{
  ...
  //{
  sv4.write(map(AA0,0,180,500,2250));
  sv5.write(map(AA1,0,180,500,2250));
  sv6.write(map(AA2,0,180,500,2250));
  if(AA3 > 0)
    sv7.write(map(AA3,0,180,500,2250));
  //}
  //else
...
void printpots() //this function needs fixing, no more pot reads
                  // REDO THE MATH FOR THIS THING, PRs DONT EXIST
{
