...
double Ca0=PI/2,Ca1=0,Cra1=PI/2,Ca2=PI/2,Cfa2=0;
double Na0=PI/2,Na1=0,Nra1=PI/2,Na2=PI/2,Nfa2=0;
...
double NNX=0,NNY=81,NNZ=81,CX=0,CY=81,CZ=81;
int bs0, bs1, bcount;
int mode,counter=0;
...
{
  //time8=millis();
  //mode = map(analogRead(PP),0,1023,7,-1);
  checkMode();
  //doNada();
  //serialToServo();
  //flatDrift();
...
void checkMode()
{
  ...
  {
    setColor(140,0,140); //purple
    whip();         //lcd prolly doesnt work for this mode
  }
  else if(mode == 6)
  {
    setColor(255,0,100); //WHITE
    x=0;y=81;z=81;
    doallchuck();
  } 
  else if(mode ==7)
...
void intro2()
{
  ...
  lazarus();
  bcount=0;
  driftToHere2dClear(0,81,81);
  delay(100);
  driftToHere2dNClear(0,81,81);
  delay(100);
  driftToHere2dClear(60,76,-27);
  delay(50);
  driftToHere2dClear(60,86,-27);
  delay(500);
  bcount++;
  delay(500);
  driftToHere2dClear(0,81,81);
  //delay(500);
  //driftToHere2dClear(-50,132,0);
  //checkMode();
  delay(100);
  driftToHere2dClear(-77,132,-23);
  delay(500);
  driftToHere2dClear(-77,132,-25);
  delay(500);
  bcount++;
  delay(500);
  driftToHere2dClear(0,81,81);
  delay(100);
  driftToHere2dNClear(61,77,-27);  //changed all these 57s from 65
  delay(100);
  driftToHere2dNClear(61,87,-27);
  delay(500);
  bcount++;
  delay(500);
  driftToHere2dNClear(61,87,-27);
  ...
  killNonClear();
  killClear();
  delay(30000);             //INCERASE A LOT
  lazarus();
  driftToHere2dNClear(-60,122,-29);
  delay(100);
  driftToHere2dNClear(-60,122,-29);
  delay(500);
  bcount++;
  ...
  //driftToHere2dClear(0,81,81);
  delay(100);
  driftToHere2dNClear(61,85,-27);
  delay(500);
  //checkMode();
  bcount++;
  delay(500);
  driftToHere2dNClear(61,85,-27);  //changed all these 57s from 65
  delay(100);
  driftToHere2dNClear(0,81,81);
  delay(500);
  //bcount++;
  //delay(500);
  //bcount++;
  driftToHere2dClear(-77,128,-27);
  delay(500);
  driftToHere2dClear(-77,138,-27);
  //delay(500);
  bcount++;
  driftToHere2dClear(0,81,81);
  delay(500);
  driftToHere2dClear(60,83,-27);
  delay(500);
  bcount++;
  driftToHere2dClear(0,81,81);
  killNonClear();
  killClear();
  delay(30000);               //INCREASE A LOR
  lazarus();
  
  checkMode();    //do i need this????
}
void intro1()
{
  lcd.setCursor(0,0);
...
void driftToHere2dClear(double XX,double YY,double ZZ)
{
  ...
  for(double j = 0; j < distanceToTravel; j+= stepMe)
  { 
     CX = C_x + (XX-C_x) * j/distanceToTravel;
     CY = C_y + (YY-C_y) * j/distanceToTravel;
     CZ = C_z + (ZZ-C_z) * j/distanceToTravel;
     whipToHere2dClear(CX, CY, CZ);
     Serial.print("   x: ");
     Serial.print(NX);
     Serial.print("   y: ");
     Serial.print(NY);
...
void whipToHere2dClear(double XCX,double YCY,double ZCZ)
{
  checkbutton();
  solveangles2dClear(XCX,YCY,ZCZ);
  //gotoangles(a0,ra1,a2);
  gotoPWMClear(Ca0,Cra1,Ca2, cl);
  //dothelcdtohere();
  printmath();  
}
void whipToHere2dNClear(double XNX,double YNY,double ZNZ)
{
  checkbutton();
  solveangles2dNClear(XNX,YNY,ZNZ);
  //gotoangles(a0,ra1,a2);
  gotoPWMNClear(Na0,Nra1,Na2, cl);
  //dothelcdtohere();
  printmath();  
}
...
void solveangles2dClear(double XCX,double YCY,double ZCZ)
{
  //double r = sqrt((XX*XX)+(YY*YY));    //bird's eye view, cartesian to polar
  //double c1 = XX/r;
  double c1 = XCX/81;
  ...
  double th0 = acos(c1);
  double OOsq = YCY*YCY + ZCZ*ZCZ;
  double OO = sqrt(OOsq);
  double QQ1 = atan2(ZCZ,YCY);
  double QQ2 = acos((OOsq + bicep*bicep - forearm*forearm)/(2*forearm*OO)); 

  Ca0 = th0;
  Ca1 = QQ1+QQ2;
  Cra1 = PI-Ca1;
  Ca2 =  Ca1 - PI/2 + acos((bicep*bicep + forearm*forearm - OOsq)/(2*bicep*bicep));
  Cfa2 = Ca2-Ca1+PI/2;
  if(bcount%2==1)
    cl=3;         //work this button crap out
  else
...
void solveangles2dNClear(double XNX,double YNY,double ZNZ)
{
  //double r = sqrt((XX*XX)+(YY*YY));    //bird's eye view, cartesian to polar
  //double c1 = XX/r;
  double c1 = XNX/81;
  ...
  double th0 = acos(c1);
  double OOsq = YNY*YNY + ZNZ*ZNZ;
  double OO = sqrt(OOsq);
  double QQ1 = atan2(ZNZ,YNY);
  double QQ2 = acos((OOsq + bicep*bicep - forearm*forearm)/(2*forearm*OO)); 

  Na0 = th0;
  Na1 = QQ1+QQ2;
  Nra1 = PI-Na1;
  Na2 =  Na1 - PI/2 + acos((bicep*bicep + forearm*forearm - OOsq)/(2*bicep*bicep));
  Nfa2 = Na2-Na1+PI/2;
  if(bcount%2==1)
    cl=3;         //work this button crap out
  else
...
{                              //@ point (0,81,81) cl:middle
  ...
  sv3.writeMicroseconds(toPWM(cl)+300);  //1906
